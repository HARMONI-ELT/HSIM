...
def spectral_res(datacube, head, grating, wavels):
	...
	try:
		...
		raise ValueError('Error: ' + grating + ' is not a valid grating. Valid options are: ' + ", ".join(sorted(config_data['gratings'].keys())))
	

	print 'Input resolution = %.1f A' % (head['SPECRES']*10000.)
	print 'Input sampling = %.1f A' % (head['CDELT3']*10000.)
	print 'Internal resolution = %.1f A' % (new_res*10000.)
	print 'Internal sampling = %.1f A' % (lamb_per_pix*10000.)
	...
	if wavels[0] <= bandws.lmin and wavels[-1] >= bandws.lmax: # cube wavelength range larger than grating choice
		print 'W: Cube wavelength range larger than grating: chopping down to grating range'
		...
		print 'W: Cube shortest wavelength shorter than grating'
		...
		print 'W: Cube longest wavelength longer than grating'
		new_wavels = np.arange(wavels[0], bandws.lmax, lamb_per_pix)
		
	else:
		raise ValueError('Error: The wavelength range of the input cube ' + str(wavels[0]) + " - " + str(wavels[1]) + " is not valid for the " + grating + " grating ("  + str(bandws.lmin) + " - " + str(bandws.lmax) + ")")
	...
	print 'Interpolating data cube - spectral'
	...
	return new_cube, head, new_wavels
...
def spatial_res(datacube, head, spax):
	...
	print 'Chosen spaxel scale: ', spax
	...
	try:
		...
		raise ValueError('Error: ' + spax + ' is not a valid spaxel scale. Valid options are: ' + ", ".join(sorted(config_data['spaxel_scale'].keys())))

	print 'Input sampling = %.2f x %.2f mas' % (head['CDELT1'], head['CDELT2'])
	...
	print 'Internal sampling = %.2f x %.2f mas' % (new_sampling_x, new_sampling_y)
...
def init_cube(datacube, grating, spax):
	...
	if os.path.isfile(datacube) == True and os.path.splitext(datacube)[1].lower() == '.fits':
		...
		try:
			...
			raise ValueError('Error. Please use FITS input file - ' + str(datacube))
	...
	#RESCALE DATACUBE TO CHOSEN SPECTRAL RESOLUTION.
        cube, head, lambs = spectral_res(cube, head, grating, lambs)
