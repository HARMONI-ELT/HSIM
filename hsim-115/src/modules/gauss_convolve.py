...
def gauss_convolve(SED, sigma, lambda_space='Linear'):
    ...
    if lambda_space=='Linear':
        ...
        if len(gauss_array) > len(lin_lambs):
            offset = (len(gauss_array) - len(lin_lambs))/2
            gauss_array = gauss_array[offset:len(gauss_array)-offset]
        ...
        print 'lin_lambs:', len(lin_lambs)
        print 'lin_ys:', len(lin_ys)
        print 'gauss_array:', len(gauss_array)
        print 'conv_ys:', len(conv_ys)
